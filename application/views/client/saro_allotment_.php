<div class="title">SARO Allotment Class Details
</div><br/>
...
<tbody>
		...
		<td>
			<select id="resp_center">
				<?php $rcount = count($resp_center)-1;
					for ($r = 0; $r<=$rcount;$r++){ ?>
					<option value="<?php echo $resp_center[$r]->id."/".$resp_center[$r]->code." - ".$resp_center[$r]->resp_desc; ?>"><?php echo $resp_center[$r]->code." - ".$resp_center[$r]->resp_desc; ?></option>
				<?php } ?>
			</select>
		</td>
		<td>
	...
	<tr>
		<td colspan="4"><input type="submit" onclick="ppa_detail('<?=$base_url?>','<?=$classID?>')" class="btn btn-primary pull-left" value="Save Entry"/><span class="right">Total : <input type="text" id="t_output" class="textbox" readonly="readonly" style="height:35px;" value=""/></span></td>
	</tr>
</tbody>
</table>
